...
# ╔═╡ 00a811f8-7bb7-46e4-8f62-a36816968118
using Plots

# ╔═╡ 72b0783d-ba1d-4721-aee8-d8b051183972
## Animation
using GLMakie

# ╔═╡ 8deda1e4-935b-4ef1-9291-063a4e593afd
using GeometryBasics: Point3f

# ╔═╡ 81e9aab2-6809-4de7-b103-11cbb4e8c95f
using Observables
...
# ╔═╡ b515fb16-fdde-44d0-8945-44614d869058
...
begin
	...
	v0 = 0 #m/s
	u0 = [x0, v0]
	#ω = sqrt(g/L - omega_slow^2-cos())
end

# ╔═╡ 0191edc1-be74-46ea-ab75-dbc5a858e1bb
function pendulum(du, u, p, t)
    theta = u[1]
    theta_dot = u[2]
	
    du[1] = theta_dot
    du[2] = ((w1 + l*sin(theta))*cos(theta)/l)*omega^2 - (g/l)*sin(theta)
end


# ╔═╡ d461a495-2b0c-4cef-8111-ee60f1ab8c4a
pendulum_problem = ODE.ODEProblem(pendulum, u0, tspan)

# ╔═╡ 513535d0-5402-4476-a451-96a65481785d
time_grid = range(0,2*pi;length = 1000)

# ╔═╡ 1edb1fde-7f44-422e-b1b4-828bbe612e1e
sol = ODE.solve(pendulum_problem,saveat = time_grid)

# ╔═╡ 219a7529-ad3b-45c0-aa31-7821a8199cb8
Plots.plot(sol, xlabel="Time (s)", ylabel="θ (rad) and θdot (rad/s)",labels=["Angle" "Angular Velocity"], legend=:bottomright)

# ╔═╡ 08d72f10-18d3-4444-9913-d61a7b65383d
Plots.plot(sol, vars=(0,1), xlabel="Time (s)", ylabel="θ (rad)")

# ╔═╡ 60be964a-7f25-49b7-b0d5-832332a6d4ce
begin
	t = sol.t
	θ = sol[1,:]
	
	# Position of pendulum mass
	x = (w1 .+ l .* sin.(θ)) .* cos.(omega .* t)
	y = (w1 .+ l .* sin.(θ)) .* sin.(omega .* t)
	z = h1 .- l .* cos.(θ)
end


# ╔═╡ f5f439d2-f7f0-4e5c-ba74-33037dad83d7
begin
	anim = @animate for i in 1:10:length(t)
	    ...
	    # --- Rotating arm ---
	    x_arm = w1*cos(omega*t[i])
	    y_arm = w1*sin(omega*t[i])
	    z_arm = h1
	    ...
	    x_mass = x_arm + l*sin(θi)*cos(omega*t[i])
	    y_mass = y_arm + l*sin(θi)*sin(omega*t[i])
	    z_mass = h1 - l*cos(θi)
	    ...
	    # draw vertical shaft
	    plot3d!(shaft_x, shaft_y, shaft_z, linewidth=4)
	
	    # draw rotating arm
	    plot3d!(arm_x, arm_y, arm_z, linewidth=4)
	
	    # draw pendulum
	    plot3d!(pend_x, pend_y, pend_z, linewidth=4)
	
	    # draw mass
	    scatter3d!([x_mass], [y_mass], [z_mass], markersize=5)
	end
end


# ╔═╡ 6f3a0f04-0736-4d91-b338-c2bb1e05f117
dt = t[2]-t[1]

# ╔═╡ 5105a4fe-9537-4806-a497-740eccc09599
gif(anim, "pendulum.gif", fps=1/dt)

# ╔═╡ 74cac05b-ed55-49a1-9771-1b325b963e9a
function pendulum2(du, u, p, t)
    theta = u[1]
    theta_dot = u[2]
	
    du[1] = theta_dot
    du[2] = ((w1 + l*sin(theta))*cos(theta)/l)*omega_fast^2 - (g/l)*sin(theta)
end


# ╔═╡ 4d083be6-77f4-4de7-9957-75d2b7a99bf7
pendulum_problemf = ODE.ODEProblem(pendulum2, u0, tspan)

# ╔═╡ 450dd4d2-0972-4f80-9600-312e5033200b
sol_f = ODE.solve(pendulum_problemf, saveat = time_grid)

# ╔═╡ 786b7950-dee9-468a-9f6a-cc440cc7ce76
Plots.plot(sol_f, xlabel="Time (s)", ylabel="θ (rad) and θdot (rad/s)",labels=["Angle" "Angular Velocity"], legend=:bottomright)

# ╔═╡ d901c23f-4942-415b-96db-7b4c8869ba24
Plots.plot(sol_f, vars=(0,1), xlabel="Time (s)", ylabel="θ (rad)")

# ╔═╡ 365bb36a-5df2-4978-9579-38a4992101e8
begin
	t_f = sol_f.t
	θ_f = sol_f[1,:]
	
	# Position of pendulum mass
	x_f = (w1 .+ l .* sin.(θ_f)) .* cos.(omega_fast .* t_f)
	y_f = (w1 .+ l .* sin.(θ_f)) .* sin.(omega_fast .* t_f)
	z_f = h1 .- l .* cos.(θ_f)
end


# ╔═╡ 801625cb-5976-4d5f-ad6a-4a319c2437a2
begin
	anim_f = @animate for i in 1:10:length(t_f)
	    ...
	    # --- Rotating arm ---
	    x_arm_f = w1*cos(omega_fast*t_f[i])
	    y_arm_f = w1*sin(omega_fast*t_f[i])
	    z_arm_f = h1
	    ...
	    x_mass_f = x_arm_f + l*sin(θ_fi)*cos(omega_fast*t_f[i])
	    y_mass_f = y_arm_f + l*sin(θ_fi)*sin(omega_fast*t_f[i])
	    z_mass_f = h1 - l*cos(θ_fi)
	
	    pend_x_f = [x_arm_f, x_mass_f]
	    pend_y_f = [y_arm_f, y_mass_f]
	    pend_z_f = [h1, z_mass_f]
	    ...
	    # draw vertical shaft
	    plot3d!(shaft_x_f, shaft_y_f, shaft_z_f, linewidth=4)
	
	    # draw rotating arm
	    plot3d!(arm_x_f, arm_y_f, arm_z_f, linewidth=4)
	
	    # draw pendulum
	    plot3d!(pend_x_f, pend_y_f, pend_z_f, linewidth=4)
	
	    # draw mass
	    scatter3d!([x_mass_f], [y_mass_f], [z_mass_f], markersize=5)
	end
end


# ╔═╡ 8d9151e0-802a-439a-86cd-d1af6a8d16f8
gif(anim_f, "pendulum.gif", fps=1/dt)
...
manifest_format = "2.0"
project_hash = "b3e08649f4cd315c7bd1bbbb2b51e8bc89213bdf"
...
git-tree-sha1 = "223f7c343c1cb436ca13587fd4da6ac94ccceb5c"
