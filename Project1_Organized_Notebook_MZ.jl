...
# ╔═╡ 93b8dc80-4102-4cbf-8f22-f47505f3b9c2
begin
	import OrdinaryDiffEq as ODE  # already in your notebook
	using Symbolics        # for Lagrange derivation
	using Plots
...
md"""# Lagrange Equations of Motion"""

# ╔═╡ baead9fe-fe44-47ea-9df6-aa685f864dc8
begin
	@independent_variables t
	@variables θ(t)
	@parameters m =0.1 L=0.15 g=9.81 Ω =2.0 w = 0.1

	# Initial conditions
    θ0 = 0.2               # initial angle (rad)
    θdot0 = 0.0            # initial angular velocity

    tspan = (0.0, 10.0)
	
	D = Differential(t)
	
	lagrangian_T = 0.5 * m * (L^2 * ((D(θ)^2)) +((w + L*sin(θ))^2*Ω^2))
	lagrangian_V = m * g * L * (1 - cos(θ))
	lagrangian_L = lagrangian_T - lagrangian_V
"""
	euler_lagrange = expand_derivatives(
	    Differential(t)(diff(lagrangian_L, D(θ))) - diff(lagrangian_L, θ)
	)
	eom_normalized = simplify(euler_lagrange / (m * L^2))  # = 0, contains θ¨, θ, Ω, g, L
	eom_normalized
"""	
end

# ╔═╡ 1780fbff-997c-42bf-92c5-4475fdce0a69
md"""# Least Action Equations of Motion"""

# ╔═╡ 3b6fd224-38f5-4d1c-8f0f-1a2493e47c18
begin
dL_dθdot = Symbolics.derivative(lagrangian_L, D(θ))
dL_dθ    = Symbolics.derivative(lagrangian_L, θ)

EL_equation_θ = expand_derivatives(D(dL_dθdot) - dL_dθ)
end

# ╔═╡ eb489e2a-f449-4acf-a3bd-b50afa257930
ode_symbolic = solve_for(EL_equation_θ, D(D(θ)))

# ╔═╡ bf49df29-15e6-449a-84b2-ce5e52331ddb
#Simplified Expression of the θddot
simplify(ode_symbolic)

# ╔═╡ 70e89f1d-59e3-4243-a56e-2c21163ec393
md"""# Data Visualization: 2D Plots"""

# ╔═╡ c05f30ff-f972-41f5-b1bc-47df0a788079
begin
	mass_bob = 0.1            # kg
	length_pendulum = .15     # m
	gravity_accel = 9.81      # m/s^2
	omega_frame = 5.0         # rad/s, constant frame rotation rate

	
	simulation_time_span = (0.0, 4*pi)  # seconds
end 

# ╔═╡ f036ab62-2e2f-4fe9-8783-8cad743199b3
begin
	#Solving for the motion of both slow and fast rotational speed
	
	function rot_pendulum!(du,u, params, t)
		length_pendulum, gravity_accel, omega_frame = params
	    θ,θ_dot = u
		
	    du[1] = θ_dot
	
	    du[2] = (omega_frame^2/length_pendulum)*cos(θ)*(mass_bob + length_pendulum*sin(θ)) - (gravity_accel/length_pendulum)*sin(θ)
	end
	omega_frame_slow = 5
	omega_frame_fast = 10 
	uθ = [0.1, 0.0] 
	
	params_slow = (length_pendulum,gravity_accel,omega_frame_slow,mass_bob)
	problem_slow = ODEProblem(rot_pendulum!, uθ, simulation_time_span, params_slow)
	solution_slow = solve(problem_slow)
	
	params_fast = (length_pendulum,gravity_accel,omega_frame_fast,mass_bob)
	problem_fast = ODEProblem(rot_pendulum!, uθ, simulation_time_span, params_fast)
	solution_fast = solve(problem_fast)
end

# ╔═╡ c4f72b1e-a4c6-4644-b2a5-0727a4fba8ec
begin	
	plot(solution_slow.t, solution_slow[1,:], label="Slow ω (rad/s)")
	plot!(solution_fast.t, solution_fast[1,:], label="Fast ω(rad/s)")
	title! = "Pendulum in Rotating Frame (Lagrange EOM)" 
	xlabel!("Time (s)")
	ylabel!("θ(t)")
end
...
begin
	gr() 
	omega = omega_frame_slow # or omega_frame_slow
	solution = solution_slow
	h_1 = 0.2 # given
    frame_count = 600
    time_values = range(solution.t[1], solution.t[end], length=frame_count)

    @gif for x in time_values
        ...
        Graph1 = plot(solution.t, solution[1,:],
                  xlabel="Time (s)",
                  ylabel="θ (rad)",
                  title="Angle vs Time",
                  legend=false)
        ...
        # Graph 2: Angular Speed
		
        Graph2 = plot(solution.t, solution[2,:],
                  xlabel="Time (s)",
                  ylabel="ω (rad/s)",
                  title="Angular Speed vs Time",
                  legend=false)

        scatter!(Graph2, [x], [θdot])
		
        # Graph 3: Pendulum Motion
		
		# Origin (rotation axis)
		origin_x = 0.0
		origin_z = h_1
        # Pivot position (rotating frame)
        pivot_x = mass_bob*cos(omega*x)
        pivot_y = mass_bob*sin(omega*x)
		pivot_z = h_1
        # Bob position
        bob_x = (mass_bob + length_pendulum*sin(θ))*cos(omega*x)
        bob_y = (mass_bob + length_pendulum*sin(θ))*sin(omega*x)
        bob_z = h_1 -length_pendulum*cos(θ)
		...
		# Draw frame arm (origin → pivot)
		plot!(Graph3, [origin_x, pivot_x],
		           [origin_z, pivot_z],
		           linewidth=3)
		
		# Draw pendulum rod (pivot → bob)
		plot!(Graph3, [pivot_x, bob_x],
		           [pivot_z, bob_z],
		           linewidth=3)
		
		# Draw pivot point
		scatter!(Graph3, [pivot_x], [pivot_z], markersize=5)
		
		# Draw bob
		scatter!(Graph3, [bob_x], [bob_z], markersize=7)
		
		# ----------------------
		# Combine Layout
		# ----------------------
		plot(Graph1, Graph2, Graph3, layout=(1,3), size=(1200,400))
...
begin
	gr() 
	omega_f = omega_frame_fast # or omega_frame_slow
	solution_f = solution_fast
    time_values_f = range(solution_f.t[1], solution_f.t[end], length=frame_count)

    @gif for x in time_values_f

        current = solution_f(x)
        θ = current[1]
        θdot = current[2]

        # Graph 1: Angle vs Time
        Graph1 = plot(solution_f.t, solution_f[1,:],
                  xlabel="Time (s)",
                  ylabel="θ (rad)",
                  title="Angle vs Time",
                  legend=false)
        ...
        Graph2 = plot(solution_f.t, solution_f[2,:],
                  xlabel="Time (s)",
                  ylabel="ω (rad/s)",
                  title="Angular Speed vs Time",
                  legend=false)
		...
		origin_z = h_1
        # Pivot position (rotating frame)
        pivot_x = mass_bob*cos(omega_f*x)
        pivot_y = mass_bob*sin(omega_f*x)
		pivot_z = h_1
        # Bob position
        bob_x = (mass_bob + length_pendulum*sin(θ))*cos(omega_f*x)
        bob_y = (mass_bob + length_pendulum*sin(θ))*sin(omega_f*x)
        bob_z = h_1 -length_pendulum*cos(θ)
...
manifest_format = "2.0"
project_hash = "b0ba08d9133aeb13f20acc47f262c0edbd814c1e"
...
git-tree-sha1 = "add4e82be4e0345e532f39f3bebe412e2496030f"
...
git-tree-sha1 = "23e804f3f8aef7e5e8abeb71319692a9683c21e1"
...
git-tree-sha1 = "baa1cdea4460a7638f58d0784b0482a35255a977"
...
git-tree-sha1 = "25d2e6cefff4c15cfd3ba1cd83b3574999e005ee"
...
git-tree-sha1 = "316fee5da39ba43ed9cd5369001453dcb89f76cc"
...
git-tree-sha1 = "bc403f057fb7732282c0cb1c89e0dd7ba78ce8c2"
...
git-tree-sha1 = "55d51bd800f45c746e5e92e16ab3e88d8df54117"
...
git-tree-sha1 = "2a8b83132a262302305f359d3b50cba598f9785e"
...
git-tree-sha1 = "e89de2c83d4c16d75570d98ca5202cf907f25947"
...
git-tree-sha1 = "5f36c8535941dfed422c376c347ce774ad2f6b03"
...
git-tree-sha1 = "37c3e62180f3f325425f5e75dfd4ce34104f2c09"
...
git-tree-sha1 = "4d6bc6b1e63c70f9699dbefcee53de75c9f734e0"
...
deps = ["BipartiteGraphs", "CommonSolve", "DocStringExtensions", "Graphs", "LinearAlgebra", "ModelingToolkitBase", "Moshi", "OffsetArrays", "OrderedCollections", "SciMLBase", "Setfield", "SparseArrays", "StateSelection", "SymbolicIndexingInterface", "SymbolicUtils", "Symbolics"]
git-tree-sha1 = "17f5e5ab38eba3fcfbe080e37b871a10987e3101"
...
git-tree-sha1 = "b836cbe4aa71643d6c7e98c92e9a5738937d7139"
...
git-tree-sha1 = "577e27597b571adcfa23c2365332155ebc95c194"
...
git-tree-sha1 = "2482a0ee31419c63be997d8added773d412c42d2"
...
git-tree-sha1 = "34828ddcbf1bef430a9bbdfb1982dc9e77faeef0"
...
[[deps.SymbolicCompilerPasses]]
deps = ["LinearAlgebra", "PreallocationTools", "StaticArrays", "SymbolicUtils"]
git-tree-sha1 = "b0d41c9e1ab4f81a6e5800bb7d2534330071d520"
uuid = "3384d301-0fbe-4b40-9ae0-b0e68bedb069"
version = "0.1.2"
weakdeps = ["LinearSolve"]

    [deps.SymbolicCompilerPasses.extensions]
